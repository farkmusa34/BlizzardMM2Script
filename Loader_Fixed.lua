...
-- TEMP: includes BlizzardPCCheck_HUI.lua diagnostic

local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local LocalPlayer = Players.LocalPlayer

print("[MM2 LOADER] Starting Blizzard MM2 v1.85.4...")

local BaseURL = "https://raw.githubusercontent.com/farkmusa34/BlizzardMM2Script/refs/heads/main/"

local ExistingMM2 = (getgenv and getgenv().MM2_V85_SPLIT) or _G.MM2_V85_SPLIT
if ExistingMM2 and ExistingMM2.Cleanup then
	print("[MM2 LOADER] Cleaning previous instance...")
	pcall(function() ExistingMM2.Cleanup() end)
	task.wait(0.3)
end
...
local function LoadModule(fileName)
	...
	if not downloadOk then
		warn("[MM2 LOADER] DOWNLOAD ERROR IN " .. fileName .. ": " .. tostring(scriptContent))
		return false
	end

	if not scriptContent or scriptContent == "" or scriptContent == "404: Not Found" then
		warn("[MM2 LOADER] INVALID/EMPTY FILE: " .. fileName)
		return false
	end

	local fn, compileError = loadstring(scriptContent)
	if not fn then
		warn("[MM2 LOADER] COMPILE ERROR IN " .. fileName .. ": " .. tostring(compileError))
		return false
	end

	local runOk, runResult = pcall(fn)
	if not runOk then
		warn("[MM2 LOADER] RUNTIME ERROR IN " .. fileName .. ": " .. tostring(runResult))
		return false
	end

	print("[MM2 LOADER] Successfully loaded: " .. fileName)
	return true
end
...
-- TEMPORARY PC DIAGNOSTIC
if not RequireModule("BlizzardPCCheck_HUI.lua") then
	warn("[MM2 LOADER] PC diagnostic failed to load.")
	return
end
print("[MM2 LOADER] PC diagnostic started.")

local MM2 = (getgenv and getgenv().MM2_V85_SPLIT) or _G.MM2_V85_SPLIT
if MM2 and MM2.Running then
	print("[MM2 LOADER] Blizzard MM2 v1.85.4 bootstrap COMPLETE.")
else
	warn("[MM2 LOADER] Bootstrap finished, but MM2 runtime was not detected.")
end

if not RequireModule("AutoTrader.lua") then
	warn("[MM2 LOADER] Blizzard loaded, but AutoTrader.lua failed.")
	return
end
print("[MM2 LOADER] AutoTrader started successfully.")

if not RequireModule("Notifier.lua") then
	warn("[MM2 LOADER] Blizzard loaded, but Notifier.lua failed.")
	return
end
print("[MM2 LOADER] Notifier started successfully.")
