...
local function ExecuteYeet(targetPlayer,successLabel)
	if FlingRunning then
		MM2.Notify("Another fling is already running.",2,"x","Fling Busy")
		return false
	end
	...
	local startTargetPosition = targetHRP.Position
	local targetMovedEnough = false
	...
	while FlingRunning and os.clock() - startTime < FLING_DURATION do
		...
		if phase == 0 then
			hrp.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(0,math.rad(targetHRP.Orientation.Y),0)
		elseif phase == 1 then
			hrp.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(math.rad(-60),math.rad(180),math.rad(180))
		else
			hrp.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(math.rad(60),math.rad(180),math.rad(180))
		end
		hrp.AssemblyAngularVelocity = Vector3.new(FLING_HUGE,FLING_HUGE,FLING_HUGE)
		if velocityPhase == 1 then
			hrp.AssemblyLinearVelocity = Vector3.new(387791264,919603648,-227394880)
		elseif velocityPhase == 2 then
			hrp.AssemblyLinearVelocity = Vector3.new(233146464,615062400,231791104)
		else
			hrp.AssemblyLinearVelocity = Vector3.new(-350938112,1164999296,265938784)
		end
		patternIndex += 1
	...
	local _,_,finalTargetHRP = GetTargetFlingCharacter(targetPlayer)
	if finalTargetHRP and (finalTargetHRP.Position - startTargetPosition).Magnitude >= FLING_SUCCESS_DISTANCE then
		targetMovedEnough = true
	end
	if CurrentFlingForce then
		pcall(function() CurrentFlingForce:Destroy() end)
...
local function FlingRole(role,label)
	for _,player in ipairs(Players:GetPlayers()) do
		if player ~= LocalPlayer and MM2.State.ServerRolesCache[player.Name] == role then
			task.spawn(function() ExecuteYeet(player,label) end)
			return
		end
	end
...
UI.CreateActionFeature(UI.FlingPage,"Fling Sheriff","Flings the current sheriff",function()
	FlingRole("Sheriff","Sheriff")
end)
UI.CreateActionFeature(UI.FlingPage,"Fling Hero","Flings the current hero",function()
	FlingRole("Hero","Hero")
end)
	...
	for _,suspect in ipairs(Players:GetPlayers()) do
		if suspect ~= LocalPlayer then
			...
			if humanoid and humanoid.Health > 0 and hrp then
				...
				local suspicious = victim ~= nil and distance <= FLING_NOTIFY_NEAR_DISTANCE
					...
					elseif now - since >= FLING_NOTIFY_CONFIRM_TIME then
						...
						if now - lastAlert >= FLING_NOTIFY_COOLDOWN then
							...
							if victim == LocalPlayer then
								MM2.Notify("Possible fling attempt by "..suspect.Name,3,"triangle-alert","Fling Warning")
							else
								MM2.Notify("Possible fling: "..suspect.Name.." -> "..victim.Name,3,"triangle-alert","Fling Warning")
							end
						end
					end
				else
...
Track(RunService.Stepped:Connect(function()
	if not Flags.AntiFling or FlingRunning then
		if next(AntiFlingCollisionOriginal) then RestoreAntiFlingPlayerCollisions() end
		return
	end
...
PlayerDropdown = UI.CreateDropdown(
	UI.FlingPage,
...
MM2.UI.PlayerTargetDropdown = PlayerDropdown

local function RefreshPlayerDropdown()
	if not PlayerDropdown then return end
	local names = GetPlayerNames()
	pcall(function() PlayerDropdown:Refresh(names) end)
	local selected = MM2.State.SelectedFlingTarget
	if selected then
		local stillExists = Players:FindFirstChild(selected.Name)
		if not stillExists then MM2.State.SelectedFlingTarget = nil end
	end
end
...
UI.CreateActionFeature(UI.FlingPage,"Stop Fling","Stops the current fling",StopFling)
...
UI.AddSection(UI.FlingPage,"Protection","Defensive fling protection")
...
UI.CreateToggle(
	...
	function(on)
		...
		DisableOtherPlayerCollisions()
		local character,humanoid,hrp = MM2.GetLocalCharacter()
		if character and hrp then KillLocalFlingVelocity(character,humanoid,hrp) end
	end
)
...
UI.CreateToggle(
	...
	"FlingNotify",
	function(on)
		if not on then table.clear(FlingNotifySuspiciousSince) end
	end
)
