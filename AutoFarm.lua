--============================================================
-- MM2 V8.8.0 - AutoFarm.lua
-- Coin Farm V12.1 contact sweep.
-- V8.8.0: After Bag Full priority/conflict fixes + underground hold.
--============================================================
...
UI.CreateToggle(UI.AutoFarmPage,"Anti Disconnect","Prevents the normal inactivity timeout during long farming sessions","AntiDisconnect",SetAntiDisconnect)
if Flags.AntiDisconnect then SetAntiDisconnect(true) end
...
local function FarmValidCoin(coin)
	return coin and FarmIsCoinServer(coin) and coin:IsDescendantOf(workspace)
		and FarmGetPosition(coin) ~= nil and FarmGetTouchObject(coin) ~= nil
end
	...
	for _,obj in ipairs(workspace:GetDescendants()) do
		if FarmValidCoin(obj) then
			...
			if pos then
				...
				if distance < bestDistance then best,bestDistance = obj,distance end
			end
		end
	end
...
local function FarmEnsureMovement()
	...
	if FarmAttachment and FarmAttachment.Parent == FarmHRP
		and FarmPositionAlign and FarmPositionAlign.Parent == FarmHRP
		and FarmUprightAlign and FarmUprightAlign.Parent == FarmHRP then
		return true
	end
	FarmDestroyMovement()
...
local function FarmApplyNoclip()
	...
	for _,obj in ipairs(FarmCharacter:GetDescendants()) do
		if obj:IsA("BasePart") then
			if FarmOriginalCollision[obj] == nil then FarmOriginalCollision[obj] = obj.CanCollide end
...
local function FarmStartNoclip()
	...
	FarmNoclipConnection = RunService.Stepped:Connect(function()
		if not AutoFarmRunning or (FarmPaused and not FarmBagLiftInProgress and not Flags.StayUndergroundAfterBagFull) then return end
		if FarmUpdateCharacter() then FarmApplyNoclip() end
	end)
end

local function FarmStopNoclip()
	if FarmNoclipConnection then FarmNoclipConnection:Disconnect(); FarmNoclipConnection = nil end
	for part,oldState in pairs(FarmOriginalCollision) do
		if part and part.Parent then pcall(function() part.CanCollide = oldState end) end
	end
	table.clear(FarmOriginalCollision)
end
...
local function FarmWake()
	...
	FarmPauseReason = nil
	if not FarmUpdateCharacter() then return false end
	if not FarmSafeReturnCFrame then FarmSafeReturnCFrame = FarmHRP.CFrame end
...
local function FarmSelectTarget(coin)
	if not FarmValidCoin(coin) or not FarmEnsureMovement() then return false end
	...
	for _,container in ipairs({character,backpack}) do
		if container then
			for _,obj in ipairs(container:GetChildren()) do
				if obj:IsA("Tool") and (obj.Name == "Knife" or obj:GetAttribute("IsKnife") == true) then return true end
			end
		end
	end
	return false
...
local function FarmFindGroundY()
	if not FarmUpdateCharacter() then return nil end
	...
	-- If the farmer is already below the map, upward ray finds the floor underside.
	local upResult = workspace:Raycast(FarmHRP.Position,Vector3.new(0,120,0),params)
	if upResult then return upResult.Position.Y end
	-- Otherwise search downward from slightly above the character.
	local downOrigin = Vector3.new(xz.X,FarmHRP.Position.Y+20,xz.Z)
	local downResult = workspace:Raycast(downOrigin,Vector3.new(0,-180,0),params)
	if downResult then return downResult.Position.Y end
	return nil
end

local function FarmMoveUnderground()
	if not FarmUpdateCharacter() or not FarmEnsureMovement() then return false end
	if not FarmNoclipConnection then FarmStartNoclip() end
	local floorY = FarmFindGroundY()
	if not floorY then return false end
	local target = Vector3.new(FarmHRP.Position.X,floorY-FARM_UNDERGROUND_OFFSET,FarmHRP.Position.Z)
	FarmPositionAlign.Position = target
	local started = os.clock()
	while AutoFarmRunning and MM2.Running and FarmBagFull and Flags.StayUndergroundAfterBagFull do
		if not FarmUpdateCharacter() or not FarmPositionAlign then break end
		FarmPositionAlign.Position = target
		if (FarmHRP.Position-target).Magnitude <= FARM_BAG_LIFT_REACHED_DISTANCE then return true end
		if os.clock()-started >= FARM_UNDERGROUND_TIMEOUT then break end
		task.wait(0.03)
	end
	return false
end
...
local function FarmRunAfterBagFullActions()
	...
	if FarmAfterBagActionBusy then return end
	FarmAfterBagActionBusy = true

	task.spawn(function()
		local killAllRan = false

		-- Priority 1: Kill All if enabled AND a knife actually exists.
		if Flags.KillAllAfterBagFull and FarmHasKnife() and MM2.Functions.KillAllOnce then
			local ok,success = pcall(MM2.Functions.KillAllOnce)
			killAllRan = ok and success == true
		end

		-- Priority 2: Shoot murderer. Retry only while the bag-full window is active.
		if Flags.ShootMurdererAfterBagFull and MM2.Functions.ShootMurderer then
			while MM2.Running and Flags.AutoFarm and FarmBagFull and Flags.ShootMurdererAfterBagFull do
				...
				if success == true or message == "No Murderer" then break end
				local retryDelay = (message == "Cooldown" or message == "Busy" or message == "No Gun") and 0.20 or 0.12
				task.wait(retryDelay)
			end
		end

		-- Priority 3: Fling only after the shoot loop is done so both don't fight.
		...
		-- Reset is a fallback. If Kill All successfully ran with a knife, do not reset.
		-- Stay Underground is mutually exclusive with Reset and therefore also blocks it.
		if Flags.ResetCharacterAfterBagFull and not Flags.StayUndergroundAfterBagFull and not killAllRan then
			task.wait(0.15)
			local character = LocalPlayer.Character
			local humanoid = character and character:FindFirstChildOfClass("Humanoid")
...
local function FarmBeginBagFullLift()
	...
	if Flags.StayUndergroundAfterBagFull then
		task.spawn(function()
			...
			-- Keep movement/noclip alive while the option is enabled and this bag remains full.
			while AutoFarmRunning and MM2.Running and FarmBagFull and Flags.StayUndergroundAfterBagFull do
				if FarmUpdateCharacter() and FarmPositionAlign then
					local floorY = FarmFindGroundY()
					if floorY then
						FarmPositionAlign.Position = Vector3.new(FarmHRP.Position.X,floorY-FARM_UNDERGROUND_OFFSET,FarmHRP.Position.Z)
					end
				end
				task.wait(0.20)
			end
	...
	task.spawn(function()
		if not FarmUpdateCharacter() or not FarmEnsureMovement() then
			FarmStopNoclip(); FarmDestroyMovement(); FarmBagLiftInProgress = false; FarmBagLiftDone = true
			return
		end
		if not FarmNoclipConnection then FarmStartNoclip() end
...
local function FarmLoop()
	...
	while AutoFarmRunning and MM2.Running do
		...
		if FarmBagLiftInProgress then task.wait(0.05); continue end
		if MM2.IsPreRoundActive and MM2.IsPreRoundActive() then FarmPause("INTERMISSION"); task.wait(0.10); continue end
		if FarmBagFull then
			if not FarmBagLiftDone then FarmBeginBagFullLift() end
			task.wait(0.10)
			continue
		end
		if not FarmUpdateCharacter() then FarmPause("NO CHARACTER"); task.wait(0.10); continue end
		if FarmHumanoid.Health <= 0 then FarmPause("NOT ALIVE"); task.wait(0.10); continue end
		if not FarmCurrentCoin then
			local coin,nearestDistance = FarmFindNearestCoin()
			if not coin then FarmPause("NO COINS"); task.wait(0.08); continue end
			if nearestDistance > FARM_MAX_START_DISTANCE then FarmPause("COINS TOO FAR"); task.wait(0.08); continue end
			if not FarmWake() then task.wait(0.05); continue end
			FarmSelectTarget(coin)
		end
		if FarmPaused then task.wait(0.02); continue end
		if not FarmCurrentCoin then continue end
		if not FarmCurrentCoin:IsDescendantOf(workspace) then FarmReleaseTarget(); continue end
		local coinPos = FarmGetPosition(FarmCurrentCoin)
		if not coinPos then FarmReleaseTarget(); continue end
		local distance = (FarmHRP.Position-coinPos).Magnitude
		if distance > FARM_MAX_TARGET_DISTANCE then FarmReleaseTarget(); continue end
		local collection = FarmCheckCollection(FarmCurrentCoin,coinPos)
		if collection == true or collection == "invalid" then FarmReleaseTarget(); continue end
...
UI.CreateToggle(UI.AutoFarmPage, "Reset Character After Bag Full", "Resets your character after bag full", "ResetCharacterAfterBagFull", function(on)
	if on and UI.SetToggleState then UI.SetToggleState("StayUndergroundAfterBagFull",false,false) end
end)
UI.CreateToggle(UI.AutoFarmPage, "Stay Underground After Bag Full", "Moves 6 studs below the map floor after bag full", "StayUndergroundAfterBagFull", function(on)
	if on and UI.SetToggleState then UI.SetToggleState("ResetCharacterAfterBagFull",false,false) end
end)

if Flags.ResetCharacterAfterBagFull and Flags.StayUndergroundAfterBagFull then
	Flags.StayUndergroundAfterBagFull = false
	if UI.SetToggleState then UI.SetToggleState("StayUndergroundAfterBagFull",false,false) end
end
...
local CoinRateCorner = Instance.new("UICorner"); CoinRateCorner.CornerRadius = UDim.new(0,11); CoinRateCorner.Parent = CoinRateCard
local CoinRateStroke = Instance.new("UIStroke"); CoinRateStroke.Color = UI.COLORS.Stroke; CoinRateStroke.Transparency = 0.45; CoinRateStroke.Parent = CoinRateCard
local CoinRateTitle = Instance.new("TextLabel")
CoinRateTitle.Size = UDim2.new(1,-28,0,20); CoinRateTitle.Position = UDim2.fromOffset(14,10); CoinRateTitle.BackgroundTransparency = 1
CoinRateTitle.TextXAlignment = Enum.TextXAlignment.Left; CoinRateTitle.Text = "Coin Rate"; CoinRateTitle.TextColor3 = UI.COLORS.Text
CoinRateTitle.TextSize = 12; CoinRateTitle.Font = Enum.Font.GothamBold; CoinRateTitle.Parent = CoinRateCard
local CoinRateValue = Instance.new("TextLabel")
CoinRateValue.Size = UDim2.new(1,-28,0,18); CoinRateValue.Position = UDim2.fromOffset(14,34); CoinRateValue.BackgroundTransparency = 1
CoinRateValue.TextXAlignment = Enum.TextXAlignment.Left; CoinRateValue.Text = "0 coins/min"; CoinRateValue.TextColor3 = UI.COLORS.Muted
CoinRateValue.TextSize = 10; CoinRateValue.Font = Enum.Font.Gotham; CoinRateValue.Parent = CoinRateCard
...
MM2.Track(RunService.Heartbeat:Connect(function()
	if not FarmStatsStartedAt or FarmStatsCoins <= 0 then CoinRateValue.Text = "0 coins/min"; return end
...
local FarmGameplayRemotes = ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("Gameplay")
...
if FarmCoinCollected and FarmCoinCollected:IsA("RemoteEvent") then
	Track(FarmCoinCollected.OnClientEvent:Connect(function(...)
		...
		if Flags.AutoFarm and FarmBagFull then FarmBeginBagFullLift() end
	end))
end
...
if FarmRoundStart and FarmRoundStart:IsA("RemoteEvent") then Track(FarmRoundStart.OnClientEvent:Connect(FarmResetBag)) end
if FarmCoinsStarted and FarmCoinsStarted:IsA("RemoteEvent") then Track(FarmCoinsStarted.OnClientEvent:Connect(FarmResetBag)) end
