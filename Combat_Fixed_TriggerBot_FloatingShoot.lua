...
UI.CreateToggle(UI.CombatPage, "TriggerBot", "Automatically fires at the live murderer", "TriggerBot")
...
-- TriggerBot timings/origin recovered from the working version diagnostics.
local TRIGGER_PART_INTERVAL = 0.22
local TRIGGER_CYCLE_INTERVAL = 0.60
local TRIGGER_ORIGIN_OFFSET = Vector3.new(1.4,1.25,-3.45)
local TriggerPartIndex = 1
local NextTriggerShot = 0
local LastTriggerTarget = nil

local function ResetTriggerCycle()
	TriggerPartIndex = 1
	NextTriggerShot = 0
...
local function GetTriggerTargetPart(character,index)
	if not character then return nil end
	if index == 1 then
...
local function GetBestCombatTarget()
	...
	for _,player in ipairs(Players:GetPlayers()) do
		if IsMurderer(player) then
			...
			if humanoid and humanoid.Health > 0 and torso and MM2.IsPositionWithinESPDistance(torso.Position) then
...
-- Kept for other combat features. TriggerBot no longer depends on this raycast.
...
local function EnsureCombatGun()
	...
	if not humanoid or humanoid.Health <= 0 or not backpackGun then return nil end
	local ok = pcall(function() humanoid:EquipTool(backpackGun) end)
...
-- Corrected firing origin: fixed HRP-local position, facing exact target position.
local function FireCombatGun(gun,targetPosition)
	...
	local originPosition = hrp.CFrame:PointToWorldSpace(TRIGGER_ORIGIN_OFFSET)
	if (targetPosition-originPosition).Magnitude <= 0.1 then return false end
	shoot:FireServer(
		CFrame.new(originPosition,targetPosition),
...
MM2.Functions.ShootMurderer = function()
	...
	ShootBusy = false
	if not ok then
		warn("[MM2 V8.6.3 SHOOT ERROR]",success)
		return false,"Error"
	end
...
local function LegitThrowOnce()
	...
	if not HasClearLineOfSight(targetPart) then return false,"Murderer Behind Wall" end
	local target = targetPart.Position
	local direction = target-hrp.Position
	direction = direction.Magnitude > 0.1 and direction.Unit or Vector3.new(0,0,-1)
...
task.spawn(function()
	while MM2.Running do
		if Flags.LegitThrow and Flags.RageThrow then SetThrowToggle("RageThrow",false) end
...
local function FindKnifeTool()
	...
	local function scan(container)
		...
		for _,tool in ipairs(container:GetChildren()) do
			if tool:IsA("Tool") and (tool.Name == "Knife" or tool:GetAttribute("IsKnife") == true) then
				return tool
			end
		end
		return nil
...
local function EnsureKnifeEquipped()
	...
	if knife.Parent ~= character then
		local ok = pcall(function() humanoid:EquipTool(knife) end)
		if not ok then return nil end
		local deadline = os.clock()+0.75
		repeat task.wait(0.03) until knife.Parent == character or os.clock() >= deadline
	end
	if knife.Parent ~= character then return nil end
	return knife
end
...
local function GetKnifeTargetsInRange(range)
	range = math.clamp(tonumber(range) or Flags.KnifeRange,KNIFE_RANGE_MIN,KNIFE_RANGE_MAX)
	local character = LocalPlayer.Character
	local hrp = character and character:FindFirstChild("HumanoidRootPart")
		...
		if player ~= LocalPlayer then
			...
			if humanoid and humanoid.Health > 0 and targetPart then
				...
				if distance <= range then
					table.insert(targets,{Player=player,Part=targetPart,Distance=distance})
				end
			end
		end
	end
...
local function TouchKnifeTargets(handle,targets)
	if not handle or not firetouchinterest then return 0 end
	local touched = {}
	for _,target in ipairs(targets) do
		local part = target.Part
		if part and part.Parent then
			local ok = pcall(function() firetouchinterest(handle,part,0) end)
			if ok then table.insert(touched,part) end
		end
	end
	if #touched > 0 then RunService.Heartbeat:Wait() end
	for _,part in ipairs(touched) do
		if part and part.Parent then pcall(function() firetouchinterest(handle,part,1) end) end
	end
	return #touched
end

local function ProcessKnifeRange(knife,range)
	if not knife or knife.Parent ~= LocalPlayer.Character then return 0 end
...
local function BindKnifeRangeActivation(knife)
	if knife == KnifeRangeBoundKnife and KnifeRangeActivatedConnection then return end
	...
	KnifeRangeActivatedConnection = knife.Activated:Connect(function()
		...
		task.spawn(function() ProcessKnifeRange(knife,Flags.KnifeRange) end)
	end)
	Track(KnifeRangeActivatedConnection)
end

MM2.Functions.GetKnifeRangeMax = function() return KNIFE_RANGE_MAX end
...
MM2.Functions.ProcessKnifeRange = function(range)
	...
	if touched <= 0 then return false,"No Targets" end
	return true,"Triggered "..touched
end

MM2.Functions.KillAllOnce = function()
	if KillAllBusy then return false,"Busy" end
	local now = os.clock()
	if now-LastKillAll < KILL_ALL_COOLDOWN then return false,"Cooldown" end
	KillAllBusy = true
	local success,message = false,"Error"
	local ok,err = pcall(function()
		local knife = EnsureKnifeEquipped()
		if not knife then message = "No Knife" return end
		local handle = GetKnifeHandle(knife)
		if not handle then message = "No Handle" return end
		local targets = GetKnifeTargetsInRange(KNIFE_RANGE_MAX)
		if #targets <= 0 then message = "No Targets" return end
		KnifeRangeSuppressActivated = true
		local activated = pcall(function() knife:Activate() end)
		if not activated then KnifeRangeSuppressActivated = false message = "Activation Failed" return end
		local touched = TouchKnifeTargets(handle,targets)
		task.delay(0.08,function() KnifeRangeSuppressActivated = false end)
		if touched <= 0 then message = "No Targets" return end
		success = true
...
local function UpdateCombatFeatures()
	...
	local targetPlayer = FindLiveMurderer()
	if not targetPlayer then
		ResetTriggerCycle()
		return
	end

	if targetPlayer ~= LastTriggerTarget then
		TriggerPartIndex = 1
		NextTriggerShot = 0
		LastTriggerTarget = targetPlayer
	end

	if not IsLivePlayer(targetPlayer) or not IsMurderer(targetPlayer) then
		ResetTriggerCycle()
		return
	end
	...
	local targetPart = GetTriggerTargetPart(targetPlayer.Character,TriggerPartIndex)
	if not targetPart or not targetPart.Parent then
		TriggerPartIndex = 1
		NextTriggerShot = now + TRIGGER_PART_INTERVAL
		return
	end

	if FireCombatGun(gun,targetPart.Position) then
		if TriggerPartIndex == 3 then
			TriggerPartIndex = 1
			NextTriggerShot = now + TRIGGER_CYCLE_INTERVAL
		else
			TriggerPartIndex += 1
			NextTriggerShot = now + TRIGGER_PART_INTERVAL
		end
...
RunService:BindToRenderStep("MM2_V8_CombatFeatures",Enum.RenderPriority.Camera.Value+1,UpdateCombatFeatures)
				...
				if input.UserInputState == Enum.UserInputState.End then
					...
					if moved then
						...
						task.delay(0.12,function()
							if button and button.Parent then button:SetAttribute("_JustDragged",false) end
						end)
					end
				end
			end))
...
Track(FloatingShootButton.MouseButton1Click:Connect(function()
	...
	if FloatingShootButton:GetAttribute("_Busy") then return end
	FloatingShootButton:SetAttribute("_Busy",true)
	task.spawn(function()
		...
		if FloatingShootButton and FloatingShootButton.Parent then
			if not ok then
				warn("[MM2 V8.6.3 SHOOT] BUTTON CALL ERROR:",success)
		...
		task.delay(0.85,function()
			if FloatingShootButton and FloatingShootButton.Parent then
				FloatingShootButton.Text = "Shoot Murderer (Rage)"
				FloatingShootButton:SetAttribute("_Busy",false)
			end
		end)
	end)
end))
...
Track(FloatingLegitShootButton.MouseButton1Click:Connect(function()
	...
	if FloatingLegitShootButton:GetAttribute("_Busy") then return end
	FloatingLegitShootButton:SetAttribute("_Busy",true)
	task.spawn(function()
		...
		task.delay(0.85,function()
			if FloatingLegitShootButton and FloatingLegitShootButton.Parent then
				FloatingLegitShootButton.Text = "Shoot Murderer (Legit)"
				FloatingLegitShootButton:SetAttribute("_Busy",false)
			end
		end)
	end)
end))
...
if UI.CreateMovableCircleButton then
	local FloatingKillAllButton,FloatingKillAllHolder = UI.CreateMovableCircleButton(
		"FloatingKillAll","💀","KILL ALL",UDim2.new(0.67,-52,0.78,-42),
		function()
			local ok,success,message = pcall(function() return MM2.Functions.KillAllOnce() end)
			if not ok then
...
MM2.Functions.UpdateAutoGrab = function()
	if not Flags.AutoGrab or MM2.IsPreRoundActive() or MM2.State.Is_Picking_Up or MM2.HasGunAnywhere() or IsLocalMurderer() then return end
	local gunDrop = MM2.State.CachedGunDrop
	if not gunDrop or not gunDrop.Parent then gunDrop = workspace:FindFirstChild("GunDrop",true) end
	...
	pcall(function()
		for _ = 1,AUTO_GRAB_TOUCH_BURST do
			if not Flags.AutoGrab or MM2.HasGunAnywhere() or not gunDrop.Parent or not targetPart.Parent then break end
