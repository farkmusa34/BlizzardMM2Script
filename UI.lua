...
local function CreateSnowflake(parent,size,color)
	...
	for _,rotation in ipairs({0,60,120,180,240,300}) do
		...
		local branchCenterX =
			cx + math.cos(rad)*(size*0.27)

		local branchCenterY =
			cy + math.sin(rad)*(size*0.27)
...
local function NewPage(name)
	...
	layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		page.CanvasSize =
			UDim2.fromOffset(0,layout.AbsoluteContentSize.Y+18)
	end)
...
function MM2.UI.ShowPage(name)
	...
	for tabName,btn in pairs(TabButtons) do
		...
		btn.BackgroundColor3 =
			active and COLORS.Card or Color3.fromRGB(0,0,0)
			...
			for _,obj in ipairs(icon:GetDescendants()) do
				if obj:IsA("Frame")
					...
					obj.BackgroundColor3 =
						active and COLORS.Accent or COLORS.Muted
...
function MM2.UI.CreateToggle(parent,titleText,description,flagName,callback)
	...
	Track(toggleTrack.MouseButton1Click:Connect(function()
		local newValue =
			not (Flags[flagName] == true)

		MM2.UI.SetToggleState(
			flagName,
			newValue,
			false
		)
...
function MM2.UI.CreateMovableCircleButton(
	name,
	icon,
	labelText,
	startPosition,
	...
	local hasIcon =
		icon ~= nil and tostring(icon) ~= ""

	button.Text =
		hasIcon
		and tostring(icon)
		or tostring(labelText or name)
...
function MM2.UI.CreateValueControl(
	parent,
	labelText,
	getter,
	setter,
...
function MM2.UI.CreateSlider(
	...
	local function snap(v)
		return math.clamp(
			math.floor(((v-minValue)/step)+0.5)
			* step
			+ minValue,
			minValue,
			maxValue
